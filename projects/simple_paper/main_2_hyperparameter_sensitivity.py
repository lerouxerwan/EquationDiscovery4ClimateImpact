...
# COLUMN NAMES
HYPERPARAMETER_NAME_COLUMN_NAME = 'Hyperparameter name'
DEFAULT_VALUE_COLUMN_NAME = 'Value set by default'
TOP_VALUE_COLUMN_NAME = 'Value leading to Top RMSE RCP4.5'
TOP_COMPLEXITY_COLUMN_NAME = 'Complexity C(f) with Top RMSE RCP4.5'
TOP_RMSE_COLUMN_NAME = 'Top RMSE RCP4.5'
RELATIVE_DIFFERENCE_COLUMN_NAME = 'Drop in RMSE RCP4.5 compared to default (\%)'
...
def compute_df(fast, model_selection):
    ...
    if fast:
        param_name_to_values = {p: v for p, v in param_name_to_values.items() if p in ['tournament_selection_p', 'tournament_selection_n']}
        # param_name_to_values = {p: v[:1] for p, v in param_name_to_values.items() if p in ['unary_operators']}
    for param_name, param_values in param_name_to_values.items():
        ...
        emulator_with_search = EmulatorWithSearch(model_selection, **{'param_grid': param_grid, 'search_style': 'grid', 'n_jobs': -1})
        fit(emulator_with_search, dataset, refit=False)
        experiment = emulator_with_search.experiment_
        params_for_search = experiment.df_cv_results['params'].values[0]
        assert len(params_for_search) == 1, params_for_search
        only_key = list(params_for_search.keys())[0]
        assert only_key == param_name, f'{only_key} != {param_name}'
        ...
        d[TOP_COMPLEXITY_COLUMN_NAME] = experiment.top_complexity
        ...
        # d[MEAN_FIT_TIME] = experiment.mean_fit_time  #

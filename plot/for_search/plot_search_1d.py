...
from emulator.utils_hyperparameter_search.utils_column_names import PARAMS_EMULATOR_COLUMN_NAME, \
    RMSE_VALIDATION_COLUMN_NAME, RMSE_TRAIN_COLUMN_NAME, NLL_TRAIN_COLUMN_NAME, NLL_VALIDATION_COLUMN_NAME
...
def _plot_search_1d(ax: Axes, df_cv_results: pd.DataFrame, param_values: list[float], model_selection: str) -> None:
    train_column_name = RMSE_TRAIN_COLUMN_NAME if RMSE_TRAIN_COLUMN_NAME in df_cv_results.columns else NLL_TRAIN_COLUMN_NAME
    validation_column_name = RMSE_VALIDATION_COLUMN_NAME if RMSE_VALIDATION_COLUMN_NAME in df_cv_results.columns else NLL_VALIDATION_COLUMN_NAME
    ...
    for sorted_param_value in sorted_param_values:
        ...
        index_min_rmse_validation = df_cv_results.loc[ind, validation_column_name].idxmin()
        min_rmse_validation_list.append(df_cv_results.loc[index_min_rmse_validation, validation_column_name])
        corresponding_rmse_train_list.append(df_cv_results.loc[index_min_rmse_validation, train_column_name])
    ax.plot(sorted_param_values, min_rmse_validation_list, label=validation_column_name.replace('_', ' '), marker='o')
    ax.plot(sorted_param_values, corresponding_rmse_train_list, label=train_column_name.replace('_', ' '), marker='o')

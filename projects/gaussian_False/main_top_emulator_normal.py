...
def main():
    for opt_type in [OptimizationRandom_400]:
        for model_selection in ['best', 'validated'][:1]:
            for validation_split in [ValidationSplit.RANDOM, ValidationSplit.QUANTILE_WITH_BINNING, ValidationSplit.EXTREME][1:2]:
                for validation_size in [0.2, 0.25, 0.3][2:]:
                    ...
                    top_emulator = opt.get_top_emulator(dataset.X_train, dataset.y_train, dataset.validation_mask,
                                         dataset.X_variable_names, dataset.X_units, dataset.y_units)

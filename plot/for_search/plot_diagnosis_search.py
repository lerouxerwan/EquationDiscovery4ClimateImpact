...
from emulator.utils_hyperparameter_search.utils_column_names import PARAMS_EMULATOR_COLUMN_NAME, \
    get_cv_results_column_name, FEATURE_INDEXES_COLUMN_NAME, RMSE_VAL_COLUMN_NAME
...
def plot_diagnosis_search(emulator: Emulator, dataset:Dataset, show: Optional[bool] = False,
    ...
    if isinstance(emulator, EmulatorValidatedWithSearch):
        _plot_diagnosis_search(emulator, dataset, emulator.experiment_, show, plot_folder)


def _plot_diagnosis_search(emulator: Emulator, dataset: Dataset, experiment: Experiment, show: bool = False,
                           plot_folder: Optional[str] = None):
    plot_selected_features_best_equation(emulator, dataset, experiment, show, plot_folder)
    for nb_top_equations in [5, 10, 20]:
        plot_selected_features_top_equations(emulator, dataset, experiment, nb_top_equations, show, plot_folder)
    # plot_diagnosis_search_1d(emulator, experiment, show, plot_folder)

def plot_selected_features_top_equations(emulator: Emulator, dataset: Dataset, experiment: Experiment,
                                         nb_top_equations: int = 10, show: bool = False,
                                         plot_folder: Optional[str] = None):
    """Plot the selected features for top equations"""
    selected_feature_index_column_name = get_cv_results_column_name(emulator.model_selection, FEATURE_INDEXES_COLUMN_NAME)
    df = experiment.df_cv_results
    if len(df) >= nb_top_equations:
        ax = plt.gca()
        # Gather feature indexes from the top 10 equations
        all_feature_indexes = []
        for selected_feature_indexes in df[selected_feature_index_column_name].values[:nb_top_equations]:
        ...
        for selected_feature_index in experiment.df_cv_results[selected_feature_index_column_name].values[0]:
...
def plot_selected_features_best_equation(emulator: Emulator, dataset: Dataset, experiment: Experiment, show: bool = False,
                                         plot_folder: Optional[str] = None):
    """Plot the selected features in the best equation"""
    selected_feature_index_column_name = get_cv_results_column_name(emulator.model_selection, FEATURE_INDEXES_COLUMN_NAME)
    selected_feature_indexes = experiment.df_cv_results[selected_feature_index_column_name].values[0]
...
def plot_diagnosis_search_1d(emulator: Emulator, experiment: Experiment, show: bool, plot_folder: Optional[str] = None):
    ...
    metric_name = get_cv_results_column_name(emulator.model_selection, RMSE_VAL_COLUMN_NAME)

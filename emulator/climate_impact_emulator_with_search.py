...
class ClimateImpactEmulatorWithSearch(ClimateImpactEmulator):
    ...
    def search_cv_kwargs(self) -> dict:
        ...
        if self.search_cv_type is GridSearchCV:
            ...
            assert isinstance(self.param_grid, dict)
            assert self.n_iter is not None
            return {'n_iter': self.n_iter, 'random_state': random_seed,
                    'param_distributions': get_param_distributions(self.param_grid)}
        else:
            raise NotImplementedError
    ...
    def __init__(self, model_selection: Literal["best", "accuracy", "score"] = "best", *,
        ...
        super().__init__(model_selection, binary_operators=binary_operators, unary_operators=unary_operators,
                         ...
                         threshold_for_best_model_selection=threshold_for_best_model_selection, **kwargs)

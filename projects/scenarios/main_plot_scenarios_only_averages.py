...
def main_plot_rcp_and_ssp_only_averages(variable_name: str, show: bool = False):
    # Display parameters
    ax = plt.gca()
    window_size = 20
    ...
    for model, scenarios in model_to_scenarios.items():
        ...
        historical_years, historical_values = get_years_and_values(model, 'HIST', variable_name, window_size)
    ...
    plot_name = f'"values"_{variable_name}'
...
if __name__ == '__main__':
    ...
    # for name in ['relative_contribution_NPP_for_shortwave_term']:
        main_plot_rcp_and_ssp_only_averages(name, show=False)
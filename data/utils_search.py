...
def get_filepath_cv_results(X_sum: float, y_sum: float, validation_size: float, search_cv_type: type, n_iter: int,
                            param_grid: dict, feature_selection_name: str, select_k_features: Optional[int]) -> str:
    folder_path = get_folder_path(X_sum, y_sum, validation_size, search_cv_type, n_iter, param_grid, feature_selection_name, select_k_features)
    return op.join(folder_path, 'cv_results.csv')

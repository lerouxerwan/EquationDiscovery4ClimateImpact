...
def get_top_experiment(X: np.ndarray, y: np.ndarray, validation_mask: np.ndarray[bool], model_selection: str) -> Experiment:
    return get_top_experiments(X, y, validation_mask, 1, model_selection)[0]

def get_top_experiments(X: np.ndarray, y: np.ndarray, validation_mask: np.ndarray[bool], nb_top_experiments: int, model_selection: str) -> list[Experiment]:
    log_info('Load top search experiments...')
    ...
    return sorted(experiments, key=lambda x: x.top_rmse_validation)[:nb_top_experiments]
